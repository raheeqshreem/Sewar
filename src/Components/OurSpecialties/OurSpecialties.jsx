...
export default function OurSpecialties() {
  ...
  // الكروت الثابتة
  const first8 = [
    {  id: 1,titleAr: "حالات العظام والعضلات", image: Bone, description: `• الديسك والانزلاق الغضروفي\n• آلام الرقبة والظهر\n• خشونة المفاصل\n• التهاب أوتار الكتف والركبة والكوع\n• تمزقات العضلات\n• ألم الكعب والتهابات القدم` },
    { id: 2, titleAr: "الإصابات الرياضية", image: Sport, description: `• إصابات الملاعب\n• قطع أو إصابة الأربطة\n• إصابات الغضاريف\n• التواء الكاحل وتمزق العضلات` },
    { id: 3, titleAr: "ما بعد الكسور والعمليات", image: Operations, description: `• تأهيل ما بعد الكسور\n• ما بعد عمليات المفاصل\n• ما بعد إصابات الأربطة\n• استعادة المشي والقوة والتوازن` },
    {  id: 4, titleAr: "حالات الأعصاب", image: Nerves, description: `• الجلطات الدماغية \n• الشلل النصفي أو الرباعي \n• إصابات الحبل الشوكي \n• شلل العصب السابع \n• الاعتلال العصبي الطرفي` },
    { id: 5, titleAr: "حالات الأطفال", image: Child, description: `• التأخر الحركي\n• الشلل الدماغي\n• خلع الورك الولادي\n• القدم المعوجة\n• المشي على أطراف الأصابع` },
    {  id: 6, titleAr: "حالات كبار السن", image: Women, description: `• صعوبة المشي\n• فقدان التوازن\n• ضعف العضلات\n• آلام المفاصل المزمنة` },
    { id: 7,  titleAr: "مشاكل القوام والعمود الفقري", image: Vertebral, description: `• تحدب الظهر\n• ميلان العمود الفقري\n• آلام الجلوس الطويل\n• آلام الرقبة بسبب الهاتف أو الكمبيوتر` },
    { id: 8, titleAr: "العلاج الطبيعي لحد باب بيتك", image: Homee, description: `• جلسات علاج طبيعي مخصصة لك في منزلك\n• راحة تامة دون الحاجة لمغادرة المنزل\n• متابعة وتقييم مستمر للتقدم` }
  ];
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const res = await axios.get("https://sewarwellnessclinic1.runasp.net/api/Services/all");
...
const handleDelete = async (id) => {
  ...
  try {
    ...
    await axios.delete(`https://sewarwellnessclinic1.runasp.net/api/Services/delete/${id}`);
...
const handleEdit = async (id, title, description, file) => {
  ...
  try {
    await axios.put(`https://sewarwellnessclinic1.runasp.net/api/Services/update/${id}`, formData, {
      headers: {
...
const allServices = [
  ...
  ...restServices.map(item => ({
    ...
    image: item.picUrl ? `https://sewarwellnessclinic1.runasp.net${item.picUrl}` : "",
    isFromBackend: true
  }))
];
        ...
        {rowItems.map((item, idx) => (
          <Col key={idx} lg={4} md={6} sm={12} className="mb-4">
            <Card
              ...
              onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-6px)"; e.currentTarget.style.boxShadow = "0 18px 40px rgba(0,0,0,0.4)"; }}
            ...
            >
              {item.image && (
                <Card.Img
                  variant="top"
                  src={item.image}
                  style={{ width: "100%", height: "180px", objectFit: "cover", borderRadius: "12px", border: "3px solid #ced1d1ff", boxShadow: "0 8px 20px rgba(0,0,0,0.2)" }}
                />
              )}
              <Card.Body className="d-flex flex-column">
                <Card.Title className="fw-bold mb-3 text-center" style={{ fontSize: "1.25rem", color: "#2a7371", textShadow: "0 2px 6px rgba(0,0,0,0.15)" }}>
                  {item.titleAr}
                </Card.Title>

                {item.description && (
                  <Card.Text className="text-muted flex-grow-1" style={{ fontSize: "0.88rem", lineHeight: "1.6", textAlign: "right", direction: "rtl", color: "#555", whiteSpace: "pre-line", paddingRight: "15px" }}>
                    {item.description.split("\n").map((line, idx) => (
                      <div key={idx} style={{ marginBottom: "6px" }}>
                        <span style={{ color: "#2a7371", marginRight: "5px" }}>✦</span>
                        {line.replace("• ", "  ")}
                      </div>
                    ))}
                  </Card.Text>
...
>
  <i className="bi bi-chat-dots" style={{ fontSize: "1rem", marginRight: "6px" }}></i>
  آراء المرضى
</button>
  ...
  <i className="bi bi-arrow-right-short" style={{ fontSize: "1rem" }}></i>
                ...
                {item.isFromBackend && isSchedulerAdmin && (
                  <div className="d-flex justify-content-center gap-3 mt-3">
                    <Trash2 size={20} color="#c0392b" style={{ cursor: "pointer" }} onClick={() => handleDelete(item.id)} />
<Edit
  size={20}
  color="#2980b9"
...
/>                  </div>
  ...
  return (
    <Container className="mt-5" style={{ minHeight: "calc(100vh - 200px)", paddingTop: "120px", paddingBottom: "80px" }}>
       {/* عنوان تخصصاتنا */}
    <div className="text-center mb-3">
      ...
      >
        <span role="img" aria-label="specialty">🩺</span> تخصصاتنا
      </h2>
    </div>
      ...
      {isSchedulerAdmin && (
        <div className="text-center mb-4 w-100">
          <button style={{ backgroundColor: "#2a7371", color: "white", padding: "10px 20px", borderRadius: "10px", border: "none", fontSize: "1rem", cursor: "pointer", marginTop: "20px" }} onClick={() => navigate("/Cards")}>
            + إضافة تخصص جديد
          </button>
        </div>
      )}
