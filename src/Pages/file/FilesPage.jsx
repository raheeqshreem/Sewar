...
const FilesPage = () => {
  ...
  const fetchFiles = async (date = null, search = "") => {
    ...
    setNoDataMessage("");
    try {
      let url = "https://sewarwellnessclinic1.runasp.net/api/FilesPage/files?all=false";
      ...
      if (response.data.length === 0) {
        if (date && search) {
          ...
          setNoDataMessage("⚠️ لا توجد ملفات مطابقة لاسم المريض أو رقم الهوية.");
        } else {
...
const handleSearch = async (term) => {
  ...
  const filteredFiles = files.filter((file) => {
    ...
    const termWithoutDate = searchTerm.replace(dateRegex, "").trim().toLowerCase();
    return (
...
const showAppointmentColumn = searchTerm.trim() === "" || isSearchingByDate;
  ...
  return (
    <div
      className="container"
      dir="rtl"
      style={{ minHeight: "calc(100vh - 200px)", paddingTop: "120px", paddingBottom: "80px" }}
    >
      <div className="card shadow-lg p-4 p-md-5 border-0 rounded-4" style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        <div className="mb-4 text-center">
          <input
            ...
            style={{ border: `2px solid ${accentColor}`, maxWidth: "100%", direction: "rtl" }}
            value={searchTerm}
        ...
        {!loading && filteredFiles.length > 0 && (
          <div className="table-responsive">
            <table className="table table-hover table-bordered text-center align-middle">
              <thead style={{ backgroundColor: accentColor, color: "#fff", fontSize: "0.9rem" }}>
                <tr>
                  <th>رقم المريض</th>
                  <th>اسم المريض</th>
{showAppointmentColumn && <th>موعد الزيارة</th>}
              ...
              <tbody>
                {filteredFiles.map((file, index) => (
                  <tr key={index}>
                    ...
                    <td data-label="اسم المريض" className="fw-semibold">{file.fullName}</td>
{showAppointmentColumn && (
  ...
  style={{ backgroundColor: accentColor, color: "#fff", fontWeight: 600 }}
  onClick={() => navigate("/visites", { state: { childId: file.id ,fullName:file.fullName, gender:file.gender} })}
>
  المواعيد
</button>
                    </td>
...
{modalData && (
  ...
  تقارير {modalData.fullName ? modalData.fullName : `تقرير ${currentPage}`}
</h6>
        </div>
  ...
  {visibleReports.map((report, idx) => {
    const name = report.reportName ? report.reportName.trim() : "";
    const isInvalidName =
      name === "" ||
      name === "غير محدد" ||
      ...
      name === null;
    const reportLabel = isInvalidName ? `تقرير ${startIdx + idx + 1}` : name;
...
>
  <input
    ...
    onChange={(e) =>
      setReportNames((prev) => ({ ...prev, [report.id]: e.target.value }))
    }
    onKeyDown={(e) => {
      if (e.key === "Enter") handleReportNameSave(report.id);
    }}
    style={{
  ...
  <button
    onClick={() => handleOpenReport(report.id, report.name)}
    style={{
      background: "transparent",
      border: "none",
      color: "#f0e4d7",
...
{totalReports > reportsPerPage && (
  <div className="d-flex justify-content-center align-items-center mt-3" style={{ flexDirection: "row" }}>
